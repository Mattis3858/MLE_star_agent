...
# Using fixed weights based on performance: 0.7 for XGBoost, 0.3 for LightGBM
xgb_weight = 0.7
lgb_weight = 0.3

y_pred_xgb = models['xgboost'].predict(X_val_scaled)
y_pred_lgb = models['lightgbm'].predict(X_val_scaled)

# Create weighted ensemble prediction
y_pred_ensemble = xgb_weight * y_pred_xgb + lgb_weight * y_pred_lgb
ensemble_mape = safe_mape(y_val, y_pred_ensemble)
print(f"Weighted Ensemble MAPE ({xgb_weight:.1f} XGBoost + {lgb_weight:.1f} LightGBM): {ensemble_mape:.4f}")

# Use the ensemble as the final model if it performs better than individual models
if ensemble_mape < min(results.values()):
    ...
    print(f"Individual {best_model_name} model selected as best model")

# Feature importance plot for XGBoost (better performing model)
