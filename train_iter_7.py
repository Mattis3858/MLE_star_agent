...
# Weighted ensemble optimization based on inverse MAPE performance
# Lower MAPE -> higher weight
lgb_mape = results['lightgbm']
xgb_mape = results['xgboost']

# Calculate weights inversely proportional to MAPE
# Add small epsilon to avoid division by zero
epsilon = 1e-8
lgb_weight = 1 / (lgb_mape + epsilon)
xgb_weight = 1 / (xgb_mape + epsilon)
...
ensemble_pred = (lgb_weight * predictions['lightgbm']) + (xgb_weight * predictions['xgboost'])
ensemble_mape = safe_mape(y_val, ensemble_pred)
print(f"Weighted Ensemble MAPE: {ensemble_mape:.4f}")

# Select best model (either individual model or ensemble)
best_model_name = min(results, key=results.get)
best_model = models[best_model_name]
final_mape = results[best_model_name]

# Check if ensemble is better than the best individual model
if ensemble_mape < final_mape:
    final_mape = ensemble_mape
    best_model_name = "weighted_ensemble"
    print("Ensemble outperforms individual models")

# Feature importance plot for best model (if it's not ensemble)
if best_model_name != "weighted_ensemble" and hasattr(best_model, 'feature_importances_'):
