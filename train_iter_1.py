...
class FeatureEngineer:
    ...
    def create_competitor_features(self, df):
        ...
        df['CompetitionOpenDate'] = pd.to_datetime(
            dict(year=df['CompetitionOpenSinceYear'], month=df['CompetitionOpenSinceMonth'], day=15)
        )
        df['CompetitionOpenSinceYear'] = df['CompetitionOpenSinceYear'].fillna(1900)
        df['CompetitionOpenSinceMonth'] = df['CompetitionOpenSinceMonth'].fillna(0)
        df['CompetitionOpenDate'] = pd.to_datetime(
    ...
    def create_promo_features(self, df):
        """Create promotional campaign features"""
        # Promo2 duration
        df['Promo2SinceYear'] = df['Promo2SinceYear'].fillna(0)
        df['Promo2SinceWeek'] = df['Promo2SinceWeek'].fillna(0)
        
        # Create Promo2 start date
        df['Promo2StartDate'] = pd.to_datetime(df['Promo2SinceYear'].astype(str) + '-' + 
