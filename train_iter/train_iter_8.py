...
def timeout_handler(timeout_duration):
    """Decorator to enforce a timeout on function execution using threading"""
    def decorator(func):
        @wraps(func)
...
# Model training and evaluation with timeout
@timeout_handler(3600)  # 1 hour timeout
...
def main():
    ...
    try:
        ...
        print(f"Training timed out: {e}")
        print("FINAL_MAPE: -1")  # Indicate timeout with -1
    except Exception as e:
        print(f"Training failed with error: {e}")
        print("FINAL_MAPE: -1")  # Indicate failure with -1
