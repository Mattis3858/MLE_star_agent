...
def engineer_features(df):
    ...
    # Holiday effects
    # Create a more detailed holiday feature
    df['StateHoliday'] = df['StateHoliday'].replace({'0': 0, 'a': 1, 'b': 2, 'c': 3}).astype('int8')
    
    # Days before/after holidays
    df['DaysAfterHoliday'] = (df['StateHoliday'] > 0).astype('int8')
    df['DaysBeforeHoliday'] = df['DaysAfterHoliday'].shift(1).fillna(0).astype('int8')
    
    # School vacation interactions
    # Create features for school vacation periods
    df['SchoolHoliday'] = df['SchoolHoliday'].astype('int8')
    df['DaysAfterSchoolHoliday'] = df['SchoolHoliday'].shift(-1).fillna(0).astype('int8')
    df['DaysBeforeSchoolHoliday'] = df['SchoolHoliday'].shift(1).fillna(0).astype('int8')
    
    # Interaction between holidays and promotions
    df['HolidayPromoInteraction'] = ((df['StateHoliday'] > 0) & (df['Promo'] == 1)).astype('int8')
    df['SchoolHolidayPromoInteraction'] = (df['SchoolHoliday'] & df['Promo']).astype('int8')
    
    # Promotional calendar patterns
    # Days since last promotion
    df['DaysSinceLastPromo'] = df.groupby('Store')['Promo'].apply(
        lambda x: x.groupby((x != x.shift()).cumsum()).cumcount()
    ).fillna(0).astype('int16')
    
    # Promo streaks (consecutive promo days)
    df['PromoStreak'] = df.groupby('Store')['Promo'].apply(
        lambda x: x * (x.groupby((x != x.shift()).cumsum()).cumcount() + 1)
    ).fillna(0).astype('int16')
    
    # Promo frequency (number of promos in the last 30 days)
    df['PromoFrequency'] = df.groupby('Store')['Promo'].transform(
        lambda x: x.rolling(30, min_periods=1).sum()
    ).fillna(0).astype('int8')
    
    # End of month/quarter/year effects
    df['IsEndOfMonth'] = (df.index.is_month_end).astype('int8')
    df['IsEndOfQuarter'] = (df.index.is_quarter_end).astype('int8')
    df['IsEndOfYear'] = (df.index.is_year_end).astype('int8')
    
    # Beginning of month/quarter/year effects
    df['IsBeginningOfMonth'] = (df.index.is_month_start).astype('int8')
    df['IsBeginningOfQuarter'] = (df.index.is_quarter_start).astype('int8')
    df['IsBeginningOfYear'] = (df.index.is_year_start).astype('int8')
...
def prepare_data(df):
    ...
    feature_cols = [
        ...
        'DaysAfterSchoolHoliday', 'DaysBeforeSchoolHoliday',
        'HolidayPromoInteraction', 'SchoolHolidayPromoInteraction',
        'DaysSinceLastPromo', 'PromoStreak', 'PromoFrequency',
        'IsEndOfMonth', 'IsEndOfQuarter', 'IsEndOfYear',
        'IsBeginningOfMonth', 'IsBeginningOfQuarter', 'IsBeginningOfYear',
        'IsWeekend'
    ]
