...
# Model training and evaluation with weighted ensemble
def train_and_evaluate(X, y):
    ...
    # Weighted ensemble optimization based on validation performance
    # Inverse of MAPE as weights (better models get higher weights)
    mape_values = np.array(list(results.values()))
    inverse_mapes = 1.0 / (mape_values + 1e-8)  # Add small epsilon to avoid division by zero
    weights = inverse_mapes / np.sum(inverse_mapes)
    
    # Create model name list to maintain order
    ...
    weighted_predictions = np.zeros_like(predictions[model_names[0]])
    for i, name in enumerate(model_names):
        weighted_predictions += weights[i] * predictions[name]
    
    # Calculate ensemble MAPE
    y_val_original = np.expm1(y_val)
    ensemble_mape = safe_mape(y_val_original, weighted_predictions)
    print(f"\nWeighted Ensemble MAPE: {ensemble_mape:.4f}")
    
    # Feature importance from XGBoost (as it performed best)
