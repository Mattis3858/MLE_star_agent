...
def engineer_features(df):
    ...
    # Days since last promotion - Fixed implementation using safe merge operations
    # Create a temporary DataFrame with only Store, Date, and Promo columns
    promo_df = df.reset_index()[['Store', 'Date', 'Promo']].copy()
    # Sort by Store and Date to ensure proper order
    promo_df = promo_df.sort_values(['Store', 'Date']).reset_index(drop=True)
    # Create a flag for when promo starts (Promo=1 after Promo=0)
    promo_df['PromoStart'] = (promo_df['Promo'] == 1) & (promo_df.groupby('Store')['Promo'].shift(1) != 1)
    # Forward fill the promo start date within each store
    promo_df['LastPromoDate'] = np.where(promo_df['PromoStart'], promo_df['Date'], pd.NaT)
    promo_df['LastPromoDate'] = promo_df.groupby('Store')['LastPromoDate'].fillna(method='ffill')
    # Calculate days since last promo
    promo_df['DaysSinceLastPromo'] = (promo_df['Date'] - promo_df['LastPromoDate']).dt.days.fillna(0)
    promo_df['DaysSinceLastPromo'] = promo_df['DaysSinceLastPromo'].apply(lambda x: max(0, x)).astype('int32')
    # Merge back to main dataframe
    df = df.merge(promo_df[['Store', 'Date', 'DaysSinceLastPromo']], left_on=['Store', df.index], right_on=['Store', 'Date'], how='left')
    df.drop(columns=['key_0'], inplace=True)  # Drop the extra key column from merge
    df['DaysSinceLastPromo'] = df['DaysSinceLastPromo'].fillna(0).astype('int32')
