...
# Model training and evaluation with checkpointing and timeout
def train_and_evaluate(X, y):
    ...
    # Model candidates with reduced iterations for faster training
    models = {
    ...
    # Set timeout for each model (300 seconds = 5 minutes)
    TIMEOUT_SECONDS = 300
    ...
    for name, model in models.items():
        ...
        try:
            ...
            # Cancel the alarm if training completed successfully
            signal.alarm(0)
            ...
            model_path = f"{name}_model_checkpoint.pkl"
            ...
            print(f"{name} training timed out after {TIMEOUT_SECONDS} seconds")
            signal.alarm(0)  # Cancel the alarm
            continue
        except Exception as e:
            print(f"{name} training failed with error: {str(e)}")
            signal.alarm(0)  # Cancel the alarm
            continue
    ...
    # Retrain best model on full dataset with timeout
    ...
    try:
        ...
        # Save final model
        model_path = f"final_{best_model_name}_model.pkl"
        ...
        print(f"Final training timed out after {TIMEOUT_SECONDS} seconds")
        signal.alarm(0)  # Cancel the alarm
        raise
    except Exception as e:
        print(f"Final training failed with error: {str(e)}")
        signal.alarm(0)  # Cancel the alarm
        raise
