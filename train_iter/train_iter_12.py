...
def engineer_features(df):
    ...
    # Holiday features
    df['IsStateHoliday'] = (df['StateHoliday'] != '0').astype('int8')
    df['IsSchoolHoliday'] = df['SchoolHoliday'].astype('int8')
    
    # Combined holiday effect
    df['IsHoliday'] = ((df['IsStateHoliday'] == 1) | (df['IsSchoolHoliday'] == 1)).astype('int8')
    
    # Days to/from holidays
    # State holidays
    state_holidays = df[df['IsStateHoliday'] == 1].index.unique()
    df['DaysToStateHoliday'] = df.index.to_series().apply(
        lambda x: min([abs((x - h).days) for h in state_holidays], default=0)
    ).astype('int16')
    df['DaysAfterStateHoliday'] = df.index.to_series().apply(
        lambda x: min([(x - h).days for h in state_holidays if (x - h).days >= 0], default=0)
    ).astype('int16')
    df['DaysBeforeStateHoliday'] = df.index.to_series().apply(
        lambda x: min([(h - x).days for h in state_holidays if (h - x).days >= 0], default=0)
    ).astype('int16')
    
    # School holidays
    school_holidays = df[df['IsSchoolHoliday'] == 1].index.unique()
    df['DaysToSchoolHoliday'] = df.index.to_series().apply(
        lambda x: min([abs((x - h).days) for h in school_holidays], default=0)
    ).astype('int16')
    df['DaysAfterSchoolHoliday'] = df.index.to_series().apply(
        lambda x: min([(x - h).days for h in school_holidays if (x - h).days >= 0], default=0)
    ).astype('int16')
    df['DaysBeforeSchoolHoliday'] = df.index.to_series().apply(
        lambda x: min([(h - x).days for h in school_holidays if (h - x).days >= 0], default=0)
    ).astype('int16')
    
    # Promo calendar patterns
    df['IsPromoDay'] = df['Promo'].astype('int8')
    
    # Days to/from promo
    promo_days = df[df['IsPromoDay'] == 1].index.unique()
    df['DaysToPromo'] = df.index.to_series().apply(
        lambda x: min([abs((x - p).days) for p in promo_days], default=0)
    ).astype('int16')
    df['DaysAfterPromo'] = df.index.to_series().apply(
        lambda x: min([(x - p).days for p in promo_days if (x - p).days >= 0], default=0)
    ).astype('int16')
    df['DaysBeforePromo'] = df.index.to_series().apply(
        lambda x: min([(p - x).days for p in promo_days if (p - x).days >= 0], default=0)
    ).astype('int16')
    
    # Interaction features
    df['PromoAndHoliday'] = ((df['IsPromoDay'] == 1) & (df['IsHoliday'] == 1)).astype('int8')
    df['PromoAndSchoolHoliday'] = ((df['IsPromoDay'] == 1) & (df['IsSchoolHoliday'] == 1)).astype('int8')
    df['PromoAndStateHoliday'] = ((df['IsPromoDay'] == 1) & (df['IsStateHoliday'] == 1)).astype('int8')
    
    # Weekend features
    df['IsWeekend'] = (df['DayOfWeek'].isin([5, 6])).astype('int8')  # Saturday=5, Sunday=6
    
    # Month start/end features
    df['IsMonthStart'] = (df['Day'] <= 7).astype('int8')
    df['IsMonthEnd'] = (df['Day'] >= 24).astype('int8')
    
    # Quarter start/end features
    df['IsQuarterStart'] = ((df['Month'].isin([1, 4, 7, 10])) & (df['Day'] <= 7)).astype('int8')
    df['IsQuarterEnd'] = ((df['Month'].isin([3, 6, 9, 12])) & (df['Day'] >= 24)).astype('int8')
    ...
    # Holiday mean encodings
    state_holiday_mean_sales = df.groupby('IsStateHoliday')['Sales'].mean()
    df['StateHoliday_mean_sales'] = df['IsStateHoliday'].map(state_holiday_mean_sales).astype('float32')
    
    school_holiday_mean_sales = df.groupby('IsSchoolHoliday')['Sales'].mean()
    df['SchoolHoliday_mean_sales'] = df['IsSchoolHoliday'].map(school_holiday_mean_sales).astype('float32')
    
    # Promo mean encoding
    promo_mean_sales = df.groupby('IsPromoDay')['Sales'].mean()
    df['Promo_mean_sales'] = df['IsPromoDay'].map(promo_mean_sales).astype('float32')
...
def prepare_data(df):
    ...
    feature_cols = [
        ...
        'DaysToStateHoliday', 'DaysAfterStateHoliday', 'DaysBeforeStateHoliday',
        'DaysToSchoolHoliday', 'DaysAfterSchoolHoliday', 'DaysBeforeSchoolHoliday',
        'IsPromoDay', 'DaysToPromo', 'DaysAfterPromo', 'DaysBeforePromo',
        'PromoAndHoliday', 'PromoAndSchoolHoliday', 'PromoAndStateHoliday',
        'IsWeekend', 'IsMonthStart', 'IsMonthEnd',
        'IsQuarterStart', 'IsQuarterEnd',
        'StateHoliday_mean_sales', 'SchoolHoliday_mean_sales', 'Promo_mean_sales'
    ]
