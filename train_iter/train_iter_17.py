...
def train_and_evaluate(X, y):
    ...
    # Calculate weights inversely proportional to MAPE scores
    mape_scores = np.array(list(results.values()))
    # Add small epsilon to avoid division by zero
    weights = 1 / (mape_scores + 1e-8)
    # Normalize weights to sum to 1
    weights = weights / np.sum(weights)
    
    # Create model names list for indexing
    ...
    ensemble_preds = np.zeros_like(predictions[model_names[0]])
    for i, name in enumerate(model_names):
        ensemble_preds += weights[i] * predictions[name]
    
    # Calculate final ensemble MAPE
    y_val_original = np.expm1(y_val)
    final_mape = safe_mape(y_val_original, ensemble_preds)
    
    print(f"\nEnsemble MAPE: {final_mape:.4f}")
    
    # Feature importance from one of the models (using XGBoost as example)
