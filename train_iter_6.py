...
# Create weighted ensemble predictions (0.7 XGBoost + 0.3 LightGBM)
y_pred_xgb = models['xgboost'].predict(X_val_scaled)
y_pred_lgb = models['lightgbm'].predict(X_val_scaled)
y_pred_ensemble = 0.7 * y_pred_xgb + 0.3 * y_pred_lgb
ensemble_mape = safe_mape(y_val, y_pred_ensemble)
print(f"ensemble MAPE: {ensemble_mape:.4f}")

# Use the weighted ensemble as the final model since it performs better
final_mape = ensemble_mape

# Feature importance plot for XGBoost model (as it has higher weight in ensemble)
